...
public:
    ...
    RoundedDouble(double val, int decimalPlaces = 8) // NOTE: округление к 8 цифрам после запятой
...
//-------------------------------------------------------------------
  ...
  //  RoundedDouble(double val) : value(val) {}
    ...
    double roundToDecimal(int decimalPlaces=8) const { // NOTE: явное округление к 8 цифрам после запятой
    //decimalPlaces=8;
        double scale = std::pow(10.0, decimalPlaces);
