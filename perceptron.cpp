...
RoundedDouble Perceptron::cost()
{
    ...
    RoundedDouble summ = 0;
    const RoundedDouble* h;
    RoundedDouble* x;
    ...
    summ =(RoundedDouble) (-1 / (RoundedDouble)m) * summ;

    // regularization
    RoundedDouble reg_summ = 0;
    ...
    reg_summ *= (RoundedDouble) lamda / 2*m;
    return summ + reg_summ;
}


void Perceptron::gradientDescent() // градиентСпуск (видимо собственно обучение)
{
    for (int it = 0; it < 50; ++it) {
        ...
        for (int i = 0; i < m; ++i) {
            RoundedDouble* example = dataSrc->getData(i);
            ...
            for (int l = 1; l < length; ++l) {
                ...
                for (int r = 0; r < current->getNodesNumber(); ++r) {
                    for (int c = 0; c < current->getInputLength(); ++c) {
                        ...
                        RoundedDouble delta = current->getError(r);
                        RoundedDouble a = current->getInput(c); // add 1 to a
                        RoundedDouble D = current->getDerivative(r,c) + delta * a;
        ...
        for (int l = 1; l < length; ++l) {
            ...
            for (int r = 0; r < current->getNodesNumber(); ++r) {
                for (int c = 0; c < current->getInputLength(); ++c) {
                    if(r != 0) {
                        RoundedDouble D = current->getDerivative(r,c) + lamda * current->getWeight(r,c);
                        current->setDerivative(r,c, D);
                    }
                    RoundedDouble dt = ((RoundedDouble) 1/m) * current->getDerivative(r,c);
                    //qDebug() << dt;
                    current->setDerivative(r,c, dt);

                    RoundedDouble w = current->getWeight(r,c) - current->getDerivative(r,c);
    ...
    for (int l = layer.length() - 1; l > 0; --l) {
        ...
        if(l != layer.length() - 1) {
            ...
            for (int i = 0; i < current->getNodesNumber()+1; ++i) {
                RoundedDouble lin_comb = 0;
                for (int j = 0; j < next->getNodesNumber(); ++j) {
                    lin_comb += next->getWeight(j, i) * next->getError(j);
                }
                RoundedDouble grad ;//= Layer::sigmoidGradient(current->getValue(i-1));
                if(i == 0) {
            ...
            for (int j = 0; j < nodeAmount; ++j) {
                RoundedDouble a = current->getValues()[j];
                RoundedDouble y = (RoundedDouble) (dataSrc->getLabelAsArray(index)[j]);
                RoundedDouble delta = a - y;
...
int Perceptron::getAnswer()
{
    ...
    RoundedDouble max = layer.last()->getValue(0);
    int index = 0;
...
void Perceptron::setInput(RoundedDouble* arr, int length)
{
    if(layer.isEmpty()) {
...
const RoundedDouble* Perceptron::getOutput()
{
    Layer* p_layer = NULL;
